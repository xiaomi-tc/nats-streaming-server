...
func (s *StanServer) handleSignals() {
	...
	signal.Notify(c, syscall.SIGINT, syscall.SIGTERM, syscall.SIGKILL, syscall.SIGHUP,syscall.SIGUSR1)
	go func() {
		for sig := range c {
			...
			// switch statement.
			switch sig {
			case syscall.SIGKILL, syscall.SIGHUP, syscall.SIGTERM, syscall.SIGINT:
				//2017-12-26
